...
{
  double pxFlight = p3(0);
  double pyFlight = p3(1);
  double pzFlight = p3(2);    
  double enFlight = TMath::Sqrt(square(pxFlight) + square(pyFlight) + square(pzFlight) + square(mass));
  reco::Candidate::LorentzVector p4Flight(pxFlight, pyFlight, pzFlight, enFlight);
  return p4Flight;
}

reco::Candidate::LorentzVector compFlightP4(const AlgebraicVector3& decayVertex, const AlgebraicVector3& prodVertex, double p, double mass)
{
  ...
  double norm = TMath::Sqrt(square(pxFlight) + square(pyFlight) + square(pzFlight));
  pxFlight *= (p/norm);
  pyFlight *= (p/norm);
  pzFlight *= (p/norm);    
  AlgebraicVector3 p3Flight(pxFlight, pyFlight, pzFlight);
  return compFlightP4(p3Flight, mass);
}
    ...
    trackBuilder_(trackBuilder)
{
  ...
  for ( std::vector<const reco::Track*>::const_iterator recTrack = recTracks_.begin();
    ...
    if ( trackHitPattern.numberOfValidTrackerHits() >= 5 &&
	 trackHitPattern.numberOfValidPixelHits() >= 1 &&
...
void matchedTauDecayType::print(std::ostream& stream) const
{
  stream << "genTau: En = " << genTauP4_.E() << ", Pt = " << genTauP4_.pt() << "," 
	 << " eta = " << genTauP4_.eta() << ", phi = " << genTauP4_.phi() << ", charge = " << genTauCharge_ << std::endl;
  ...
  std::pair<double, double> X_DEBUG = gjAngleToX(angle(genVisP4_, genTauP4_), genVisP4_.mass(), genInvisP4_.mass(), genVisP4_.P(), genVisP4_.E(), tauLeptonMass, isValidSolution);
  stream << " X = " << (genVisP4_.E()/genTauP4_.E()) << std::endl;
  stream << "DEBUG: solution_1 = " << X_DEBUG.first 
	 << " (gjAngle = " << gjAngleFromX_new(X_DEBUG.first, genVisP4_.mass(), genInvisP4_.mass(), genVisP4_.P(), genVisP4_.E(), tauLeptonMass, isValidSolution) << ")," 
	 << " solution_2 = " << X_DEBUG.second 
	 << " (gjAngle = " << gjAngleFromX_new(X_DEBUG.second, genVisP4_.mass(), genInvisP4_.mass(), genVisP4_.P(), genVisP4_.E(), tauLeptonMass, isValidSolution) << ")" << std::endl;
  stream << " vis(rf): P = " << boostToCOM(genTauP4_, genVisP4_).P() << std::endl;
  double visMass = genVisP4_.mass();
  double invisMass = genInvisP4_.mass();
  double pVis_lab = genVisP4_.P();
  stream << " angle(vis,tau) = " << angle(genVisP4_, genTauP4_) << " (max. gjAngle = " << gjAngle_max(visMass, invisMass, pVis_lab, tauLeptonMass) << ")" << std::endl;
  ...
  double dxFlight = genTauDecayVertexPos_(0) - genTauProdVertexPos_(0);
  double dyFlight = genTauDecayVertexPos_(1) - genTauProdVertexPos_(1);
  double dzFlight = genTauDecayVertexPos_(2) - genTauProdVertexPos_(2);
  double dFlight  = TMath::Sqrt(square(dxFlight) + square(dyFlight) + square(dzFlight));
  stream << " flight-path: dx = " << dxFlight << ", dy = " << dyFlight << ", dz = " << dzFlight 
	 << " (eta = " << p4Flight.eta() << ", phi = " << p4Flight.phi() << ", distance = " << dFlight << ")" << std::endl;  
  double gjAngle_lab = angle(p4Flight, genVisP4_);
  double enVis_lab = genVisP4_.E();
  std::pair<double, double> X = gjAngleToX(gjAngle_lab, visMass, invisMass, pVis_lab, enVis_lab, tauLeptonMass, isValidSolution);
  stream << " angle wrt. vis = " << angle(p4Flight, genVisP4_) << ", corresponding to X: solution_1 = " << X.first << ", solution_2 = " << X.second << std::endl;
  int errorFlag = 0;
  ...
  if ( hasRecTauDecayVertex_ ) {
    ...
    AlgebraicVector3 u1, u2, u3;
    compLocalCoordinates(tauFlightPath_unit, u1, u2, u3); 
    AlgebraicVector3 residual_U = transformToLocalCoordinatesPos(recDecayVertex_wrt_genDecayVertex, u1, u2, u3);
    AlgebraicMatrix33 track_pcaCov_U = transformToLocalCoordinatesCov(recTauDecayVertexCov_, u1, u2, u3); 
    ...
    std::cout << " dU1 = " << residual_U(0) << " +/- " << TMath::Sqrt(track_pcaCov_U(0, 0)) << "," 
	      << " dU2 = " << residual_U(1) << " +/- " << TMath::Sqrt(track_pcaCov_U(1, 1)) << ","
	      << " dU3 = " << residual_U(2) << " +/- " << TMath::Sqrt(track_pcaCov_U(2, 2)) << ")" << std::endl;

...
patElectronSelConfiguratorForSVfitTrackLikelihood = objSelConfigurator(
    ...
    doSelIndividual = False/data1/veelken/tmp/trash/CMSSW_5_3_13/src/TauAnalysis/SVfit/
)
    ...
    for integrationOption in [ 'mean5sigmaWithinMax' ]:
        for applyJacobiFactors in [ True, False ]:
            for fixRecToGenVertex in [ False ]:
                ...
                print "processing likelihood option = %s, integration option = %s (%s), vertex = %s" % (likelihoodOption, integrationOption, applyJacobiFactors_string, fixRecToGenVertex_string)
                ...
                svFitProducerModuleVEGAS.config.event.resonances.A.likelihoodFunctions = cms.VPSet()
                ...
                pluginNameVEGAS = "svFitProducerByIntegrationW%s%s%s%s" % \
                  (likelihoodOption, customCapitalize(integrationOption), applyJacobiFactors_string, customCapitalize(fixRecToGenVertex_string))
                svFitProducerModuleVEGAS.algorithm.pluginName = cms.string(pluginNameVEGAS)
                svFitProducerModuleVEGAS.algorithm.applyJacobiFactors = cms.bool(applyJacobiFactors)
                svFitProducerModuleVEGAS.algorithm.verbosity = cms.int32(1)
                svFitProducerModuleNameVEGAS = "svFitProducerByIntegrationW%s%s%s%s" % \
                  (likelihoodOption, customCapitalize(integrationOption), applyJacobiFactors_string, customCapitalize(fixRecToGenVertex_string))
                ...
                keyVEGAS = "%s%s%s%sVEGAS" % (likelihoodOption, customCapitalize(integrationOption), applyJacobiFactors_string, customCapitalize(fixRecToGenVertex_string))
                svFitProducerModuleNames[keyVEGAS] = svFitProducerModuleNameVEGAS
                svFitAnalyzerModuleTypes[keyVEGAS] = svFitAnalyzerModuleTypeVEGAS
                ...
                pluginNameMarkovChain = "svFitProducerByIntegration2W%s%s%s%s" % \
                  (likelihoodOption, customCapitalize(integrationOption), applyJacobiFactors_string, customCapitalize(fixRecToGenVertex_string))
                svFitProducerModuleMarkovChain.algorithm.pluginName = cms.string(pluginNameMarkovChain)
                svFitProducerModuleMarkovChain.algorithm.applyJacobiFactors = cms.bool(applyJacobiFactors)
                ...
                svFitProducerModuleNameMarkovChain = "svFitProducerByIntegration2W%s%s%s%s" % \
                  (likelihoodOption, customCapitalize(integrationOption), applyJacobiFactors_string, customCapitalize(fixRecToGenVertex_string))
                ...
                keyMarkovChain = "%s%s%s%sMarkovChain" % (likelihoodOption, customCapitalize(integrationOption), applyJacobiFactors_string, customCapitalize(fixRecToGenVertex_string))
                svFitProducerModuleNames[keyMarkovChain] = svFitProducerModuleNameMarkovChain
                svFitAnalyzerModuleTypes[keyMarkovChain] = svFitAnalyzerModuleTypeMarkovChain
            ...
            for integrationOption in [ 'mean5sigmaWithinMax' ]:
                for applyJacobiFactors in [ True, False ]:
                    for fixRecToGenVertex in [ False ]:
                        ...
                        option1Name = "%s%s%s%s%s" % (option1, customCapitalize(integrationOption), applyJacobiFactors_string, customCapitalize(fixRecToGenVertex_string), integrationMethod)
                        ...
                        option2Name = "%s%s%s%s%s" % (option2, customCapitalize(integrationOption), applyJacobiFactors_string, customCapitalize(fixRecToGenVertex_string), integrationMethod)
...
process.savePlots = cms.EDAnalyzer("DQMSimpleFileSaver",
    outputFileName = cms.string("/data1/veelken/tmp/svFitStudies/testSVfitTrackLikelihoods_3prong3prong_plots_%s_%s_%i_2013Aug14.root" % (sample_type, channel, massPoint))
)

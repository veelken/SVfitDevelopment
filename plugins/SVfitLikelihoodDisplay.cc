#include "TauAnalysis/SVfitDevelopment/plugins/SVfitLikelihoodDisplay.h"
...
#include "TauAnalysis/SVfitDevelopment/interface/svFitLikelihoodDisplayAuxFunctions.h"
  ...
  : moduleLabel_(cfg.getParameter<std::string>("@module_label"))
{
  ...
  eventVertexFitAlgorithm_ = new SVfitEventVertexRefitter(cfgEventVertexFitAlgo);
  ...
  decayVertexFitAlgorithm_ = new SVfitDecayVertexFitter(cfgDecayVertexFitAlgorithm);
  ...
  makePlots_vs_X_       = cfg.getParameter<bool>("makePlots_vs_X");
  makePlots_vs_GJangle_ = cfg.getParameter<bool>("makePlots_vs_GJangle");
...
namespace
{
  ...
  {
    // CV: code taken from TauAnalysis/SVfit/src/SVfitTauDecayBuilder.cc
...
//   (angle of visible decay products wrt. tau lepton flight direction)
    double visEnFracX = 0.;
    double gjAngle_lab = 0.;
    if ( isX ) {
      visEnFracX = gjAngle_or_X;
      gjAngle_lab = gjAngleFromX_new(visEnFracX, visMass, invisMass, pVis_lab, enVis_lab, motherMass, isValidSolution);
    } else {
...
//        i.e. is identical in tau rest-frame and laboratory frame)
      ...
      gjAngle_lab = TMath::ASin(pVis_perp/pVis_lab);
      ...
      visEnFracX = enVis_lab/enTau_lab;
    }
...
//--- compute tau lepton energy and momentum in laboratory frame  
    ...
    gjAngle_rf = TMath::ATan2(pVis_perp, pVis_parl_rf);
...
namespace
{
  ...
  {
    // CV: code copied from TauAnalysis/SVfit/plugins/SVfitTauDecayLikelihoodTrackInfo.cc
...
//    in case the flight correction moves the SV 'behind' the primary vertex.
  ...
  double negLogLikelihoodTauToLepDecay(double gjAngle, double X, double invisMass, 
				       int verbosity)
  {
    // CV: code copied from TauAnalysis/SVfit/plugins/SVfitTauToLepLikelihoodMatrixElement.cc
  ...
  double negLogLikelihoodTauToHadDecay(double gjAngle, double pVis_rf, double X, double visMass, 
				       int verbosity)
  {
    // CV: code copied from TauAnalysis/SVfit/plugins/SVfitTauToHadLikelihoodPhaseSpace.cc
  ...
  double negLogLikelihoodMEt(const reco::Candidate::LorentzVector& genMEt, const reco::Candidate::LorentzVector& recMEt, 
			     ...
			     int verbosity)
  {
    // code copied from TauAnalysis/SVfit/plugins/SVfitEventLikelihoodMEt2.cc
...
namespace
{
			  ...
			  int verbosity)
  {
    ...
    std::cout << " doVaryTheta1 = " << doVaryTheta1 << std::endl;
    std::cout << " doVaryX1 = " << doVaryX1 << std::endl;
    std::cout << " doVaryPhi1 = " << doVaryPhi1 << std::endl;
    std::cout << " doVaryInvisMass1 = " << doVaryInvisMass1 << std::endl;
    std::cout << " doVaryTheta2 = " << doVaryTheta2 << std::endl;
    std::cout << " doVaryX2 = " << doVaryX2 << std::endl;
    std::cout << " doVaryPhi2 = " << doVaryPhi2 << std::endl;
    std::cout << " doVaryInvisMass2 = " << doVaryInvisMass2 << std::endl;
    ...
    if ( doVaryX1 ) {
      ...
      std::cout << "X1(true) = " << gjAngle_or_X1_true << ", gjAngle1(true) = " << gjAngleFromLabMomenta(genP4Tau1, genP4Vis1) << std::endl;
      isX1                  = true;
    } else {
      gjAngle_or_X1_true    = gjAngleFromLabMomenta(genP4Tau1, genP4Vis1);
      std::cout << "gjAngle1(true) = " << gjAngle_or_X1_true << ", X1(true) = " << (genP4Vis1.E()/genP4Tau1.E()) << std::endl;
      isX1                  = false;
    }
    ...
    if ( doVaryX2 ) {
      ...
      std::cout << "X2(true) = " << gjAngle_or_X2_true << ", gjAngle2(true) = " << gjAngleFromLabMomenta(genP4Tau2, genP4Vis2) << std::endl;
      isX2                  = true;
    } else {
      gjAngle_or_X2_true    = gjAngleFromLabMomenta(genP4Tau2, genP4Vis2);
      std::cout << "gjAngle2(true) = " << gjAngle_or_X2_true << ", X2(true) = " << (genP4Vis2.E()/genP4Tau2.E()) << std::endl;
      isX2                  = false;
    }
	  ...
	  //for ( double gjAngle_or_X2 = 0.67784; gjAngle_or_X2 <= 1.0; gjAngle_or_X2 += 1.0 ) {
		...
		if ( addLikelihoodTrackInfo1 ) {
		  if ( hasRecDecayVertex1 ) negLogP += negLogLikelihoodTrackInfo3Prong(
		    eventVertexPos, eventVertexCov, p4Tau1.P(), refP4Vis1, tauFlightPath1_unit, genTauCharge1, 
		    recDecayVertexPos1, recDecayVertexCov1, sfProdVertexCov, sfDecayVertexCov, verbosity);
		  else negLogP += negLogLikelihoodTrackInfo1Prong(
		    eventVertexPos, eventVertexCov, p4Tau1.P(), refP4Vis1, tauFlightPath1_unit, genTauCharge1, 
		    *recLeadTrack1_trajectory, recLeadTrackRefPoint1, recLeadTrackDirection1, sfProdVertexCov, sfDecayVertexCov, verbosity);
		}
		...
		if ( addLikelihoodTrackInfo2 ) {
		  if ( hasRecDecayVertex2 ) negLogP += negLogLikelihoodTrackInfo3Prong(
		    eventVertexPos, eventVertexCov, p4Tau2.P(), refP4Vis2, tauFlightPath2_unit, genTauCharge2, 
		    recDecayVertexPos2, recDecayVertexCov2, sfProdVertexCov, sfDecayVertexCov, verbosity);
		  else negLogP += negLogLikelihoodTrackInfo1Prong(
		    eventVertexPos, eventVertexCov, p4Tau2.P(), refP4Vis2, tauFlightPath2_unit, genTauCharge2, 
		    *recLeadTrack2_trajectory, recLeadTrackRefPoint2, recLeadTrackDirection2, sfProdVertexCov, sfDecayVertexCov, verbosity);
		}
  ...
  pat::CandKinResolution::LorentzVector convertToP4PAT(const AlgebraicVector3& p3, double mass)
  {
    double px = p3(0);
    double py = p3(1);
    double pz = p3(2);
    double en = TMath::Sqrt(px*px + py*py + pz*pz + mass*mass);
    pat::CandKinResolution::LorentzVector p4(px, py, pz, en);
    return p4;
  }
  AlgebraicSymMatrix44 convertToCovPAT(const AlgebraicMatrix33& cov)
  {
    AlgebraicSymMatrix44 cov4x4;
    for ( int iRow = 0; iRow < 4; ++iRow ) {
      for ( int iColumn = 0; iColumn < 4; ++iColumn ) {
	if ( iRow < 3 && iColumn < 3 ) cov4x4(iRow, iColumn) = cov(iRow, iColumn);
	else cov4x4(iRow, iColumn) = 0.;
      }
    }
    return cov4x4;
  }
...
void SVfitLikelihoodDisplay::analyze(const edm::Event& evt, const edm::EventSetup& es)
{
  ...
  std::cout << "matchedTau #1:" << std::endl;
  matchedTau1->print(std::cout);
  std::cout << "matchedTau #2:" << std::endl;
  ...
  const reco::Vertex& recProductionVertex = vertices->front();
  std::cout << "event vertex(rec):" 
	    << " x = " << recProductionVertex.x() << " +/- " << TMath::Sqrt(recProductionVertex.covariance()(0, 0)) << ","
	    << " y = " << recProductionVertex.y() << " +/- " << TMath::Sqrt(recProductionVertex.covariance()(1, 1)) << ","
	    << " z = " << recProductionVertex.z() << " +/- " << TMath::Sqrt(recProductionVertex.covariance()(2, 2)) << ","
	    << " (chi2 = " << recProductionVertex.normalizedChi2() << ")" << std::endl;
  AlgebraicVector3 recProductionVertexPos(recProductionVertex.x(), recProductionVertex.y(), recProductionVertex.z());
  const AlgebraicVector3& genProductionVertexPos = matchedTau1->genTauProdVertexPos_;
  AlgebraicVector3 recProductionVertex_wrt_genProductionVertex = recProductionVertexPos - genProductionVertexPos;
  std::cout << "distance(rec-gen):" 
	    << " x = " << recProductionVertex_wrt_genProductionVertex(0) << " +/- " << TMath::Sqrt(recProductionVertex.covariance()(0, 0)) << "," 
	    << " y = " << recProductionVertex_wrt_genProductionVertex(1) << " +/- " << TMath::Sqrt(recProductionVertex.covariance()(1, 1)) << "," 
	    << " z = " << recProductionVertex_wrt_genProductionVertex(2) << " +/- " << TMath::Sqrt(recProductionVertex.covariance()(2, 2)) << std::endl;
  ...
  TransientVertex recProductionVertex_refitted = eventVertexFitAlgorithm_->refit(&recProductionVertex, &tauTracks);
  AlgebraicVector3 recProductionVertexPos_refitted;
  AlgebraicMatrix33 recProductionVertexCov_refitted;
  if ( recProductionVertex_refitted.isValid() ) {
    recProductionVertexPos_refitted = AlgebraicVector3(recProductionVertex_refitted.position().x(), recProductionVertex_refitted.position().y(), recProductionVertex_refitted.position().z());
    recProductionVertexCov_refitted = recProductionVertex_refitted.positionError().matrix_new();
    std::cout << "event vertex(rec,refitted):" 
	      << " x = " << recProductionVertexPos_refitted(0) << " +/- " << TMath::Sqrt(recProductionVertexCov_refitted(0, 0)) << ","
	      << " y = " << recProductionVertexPos_refitted(1) << " +/- " << TMath::Sqrt(recProductionVertexCov_refitted(1, 1)) << ","
	      << " z = " << recProductionVertexPos_refitted(2) << " +/- " << TMath::Sqrt(recProductionVertexCov_refitted(2, 2)) 
	      << " (chi2 = " << recProductionVertex_refitted.normalisedChiSquared() << ")" << std::endl;    
    AlgebraicVector3 recProductionVertex_wrt_genProductionVertex = recProductionVertexPos_refitted - genProductionVertexPos;
    std::cout << "distance(rec-gen):" 
	      << " x = " << recProductionVertex_wrt_genProductionVertex(0) << " +/- " << TMath::Sqrt(recProductionVertexCov_refitted(0, 0)) << "," 
	      << " y = " << recProductionVertex_wrt_genProductionVertex(1) << " +/- " << TMath::Sqrt(recProductionVertexCov_refitted(1, 1)) << "," 
	      << " z = " << recProductionVertex_wrt_genProductionVertex(2) << " +/- " << TMath::Sqrt(recProductionVertexCov_refitted(2, 2)) << std::endl;
    int idx = 0;
    for ( std::vector<matchedTauDecayType*>::const_iterator matchedTauDecay = matchedTauDecays.begin();
	  matchedTauDecay != matchedTauDecays.end(); ++matchedTauDecay ) {
      std::cout << "matchedTau #" << idx << ":" << std::endl;
      AlgebraicVector3 recDecayVertex_wrt_recProductionVertex = (*matchedTauDecay)->recTauDecayVertexPos_ - recProductionVertexPos_refitted;
      reco::Candidate::LorentzVector p4Flight = compFlightP4(recDecayVertex_wrt_recProductionVertex, 0.);
      pat::CandKinResolution::LorentzVector p4FlightPAT = convertToP4PAT(recDecayVertex_wrt_recProductionVertex, 0.);
      AlgebraicMatrix33 recDecayVertex_wrt_recProductionVertexCov = (*matchedTauDecay)->recTauDecayVertexCov_;
      for ( unsigned iRow = 0; iRow < 3; ++iRow ) {
	for ( unsigned iColumn = 0; iColumn < 3; ++iColumn ) {
	  recDecayVertex_wrt_recProductionVertexCov(iRow, iColumn) += recProductionVertexCov_refitted(iRow, iColumn);
	}
      }
      AlgebraicSymMatrix44 recDecayVertex_wrt_recProductionVertexCovPAT = convertToCovPAT(recDecayVertex_wrt_recProductionVertexCov);
      double etaErr = pat::helper::ResolutionHelper::getResolEta(pat::CandKinResolution::MCCart, recDecayVertex_wrt_recProductionVertexCovPAT, p4FlightPAT);
      double phiErr = pat::helper::ResolutionHelper::getResolPhi(pat::CandKinResolution::MCCart, recDecayVertex_wrt_recProductionVertexCovPAT, p4FlightPAT);
      double angle_wrt_vis = angle(p4Flight, (*matchedTauDecay)->recVisP4_);
      AlgebraicVector3 tauFlightPath_unit = normalize(AlgebraicVector3(p4Flight.px(), p4Flight.py(), p4Flight.pz()));
      AlgebraicVector3 u1, u2, u3;
      compLocalCoordinates(tauFlightPath_unit, u1, u2, u3); 
      AlgebraicVector3 recDecayVertex_wrt_recProductionVertex_U = transformToLocalCoordinatesPos(recDecayVertex_wrt_recProductionVertex, u1, u2, u3);
      AlgebraicMatrix33 recDecayVertex_wrt_recProductionVertexCov_U = transformToLocalCoordinatesCov(recDecayVertex_wrt_recProductionVertexCov, u1, u2, u3);
      AlgebraicVector3 recVisP3_unit = normalize(AlgebraicVector3((*matchedTauDecay)->recVisP4_.px(), (*matchedTauDecay)->recVisP4_.py(), (*matchedTauDecay)->recVisP4_.pz()));
      AlgebraicVector3 v1, v2, v3;
      compLocalCoordinates(recVisP3_unit, v1, v2, v3); 
      AlgebraicVector3 recDecayVertex_wrt_recProductionVertex_V = transformToLocalCoordinatesPos(recDecayVertex_wrt_recProductionVertex, v1, v2, v3);
      AlgebraicMatrix33 recDecayVertex_wrt_recProductionVertexCov_V = transformToLocalCoordinatesCov(recDecayVertex_wrt_recProductionVertexCov, v1, v2, v3);
      double dT_V = TMath::Sqrt(square(recDecayVertex_wrt_recProductionVertex_V(0)) + square(recDecayVertex_wrt_recProductionVertex_V(1)));
      double dz_V = recDecayVertex_wrt_recProductionVertex_V(2);
      double dTErr_V = TMath::Sqrt(recDecayVertex_wrt_recProductionVertexCov_V(0, 0) + recDecayVertex_wrt_recProductionVertexCov_V(1, 1));
      double dzErr_V = TMath::Sqrt(recDecayVertex_wrt_recProductionVertexCov_V(2, 2));
      //std::cout << "dT_V = " << dT_V << " +/- " << dTErr_V << std::endl;
      //std::cout << "dz_V = " << dz_V << " +/- " << dzErr_V << std::endl;
      double angle_wrt_visErr = TMath::Sqrt(square(dTErr_V)*square(dz_V) + square(dT_V)*square(dzErr_V))/(square(dT_V) + square(dz_V));
      std::cout << " flight-path:" 
		<< " dx = " << recDecayVertex_wrt_recProductionVertex(0) << " +/- " << TMath::Sqrt(recDecayVertex_wrt_recProductionVertexCov(0, 0)) << ","
		<< " dy = " << recDecayVertex_wrt_recProductionVertex(1) << " +/- " << TMath::Sqrt(recDecayVertex_wrt_recProductionVertexCov(1, 1)) << ","
		<< " dz = " << recDecayVertex_wrt_recProductionVertex(2) << " +/- " << TMath::Sqrt(recDecayVertex_wrt_recProductionVertexCov(2, 2))
		<< " (eta = " << p4Flight.eta() << " +/- " << etaErr << ", phi = " << p4Flight.phi() << " +/- " << phiErr << "," 
		<< " distance = " << recDecayVertex_wrt_recProductionVertex_U(2) << " +/- " << TMath::Sqrt(recDecayVertex_wrt_recProductionVertexCov_U(2, 2)) << ")" << std::endl;
      std::cout << " angle wrt. vis = " << angle_wrt_vis << " +/- " << angle_wrt_visErr << ")" << std::endl;
      double visMass = (*matchedTauDecay)->recVisP4_.mass();
      double pVis_lab = (*matchedTauDecay)->recVisP4_.P();
      double enVis_lab = (*matchedTauDecay)->recVisP4_.E();
      bool isValidSolution = true;
      std::pair<double, double> X     = gjAngleToX(angle_wrt_vis,                                    visMass, 0., pVis_lab, enVis_lab, tauLeptonMass, isValidSolution);
      std::cout << "X: solution_1 = " << X.first << ", solution_2 = " << X.second << std::endl;
      std::pair<double, double> Xup   = gjAngleToX(angle_wrt_vis + angle_wrt_visErr,                 visMass, 0., pVis_lab, enVis_lab, tauLeptonMass, isValidSolution);
      std::cout << "Xup: solution_1 = " << Xup.first << ", solution_2 = " << Xup.second << std::endl;
      std::pair<double, double> Xdown = gjAngleToX(TMath::Max(0., angle_wrt_vis - angle_wrt_visErr), visMass, 0., pVis_lab, enVis_lab, tauLeptonMass, isValidSolution);
      std::cout << "Xdown: solution_1 = " << Xdown.first << ", solution_2 = " << Xdown.second << std::endl;
      double X_1    = TMath::Max(X.first, X.second);
      double Xmax_1 = ( Xdown.first  >= X_1 ) ? Xdown.first  : Xdown.second;
      double Xmin_1 = ( Xup.first    <= X_1 ) ? Xup.first    : Xup.second;
      double X_2    = TMath::Min(X.first, X.second);
      double Xmin_2 = ( Xdown.second <= X_2 ) ? Xdown.second : Xdown.first;
      double Xmax_2 = ( Xup.second   >= X_2 ) ? Xup.second   : Xup.first;
      std::cout << " expected X: solution_1 = " << X_1 << " + " << (Xmax_1 - X_1) << " - " << (X_1 - Xmin_1) << ","
		<< " solution_2 = " << X_2 << " + " << (Xmax_2 - X_2) << " - " << (X_2 - Xmin_2) << std::endl;
      ++idx;
    }
  } else {
  ...
  if ( makePlots_vs_X_ ) {    
    makeLikelihoodPlot(matchedTau1->genVisP4_, matchedTau1->genInvisP4_, matchedTau1->genTauDecayMode_, matchedTau1->genTauCharge_, matchedTau1->genTauProdVertexPos_, matchedTau1->genTauDecayVertexPos_, 
		       ...
		       false, false, 
		       recProductionVertexPos_refitted, recProductionVertexCov_refitted,
		       recMEt.p4(), recMEtCov,
		       false,

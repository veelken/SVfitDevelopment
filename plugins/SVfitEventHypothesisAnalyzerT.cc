#include "TauAnalysis/SVfitDevelopment/plugins/SVfitEventHypothesisAnalyzerT.h"
...
template <typename T>
SVfitEventHypothesisAnalyzerT<T>::SVfitEventHypothesisAnalyzerT(const edm::ParameterSet& cfg)
...
template <typename T>
SVfitEventHypothesisAnalyzerT<T>::~SVfitEventHypothesisAnalyzerT()
{
...
template <typename T>
void SVfitEventHypothesisAnalyzerT<T>::beginJob()
{
...
template <typename T>
void SVfitEventHypothesisAnalyzerT<T>::analyze(const edm::Event& evt, const edm::EventSetup& es)
{
  //std::cout << "<SVfitEventHypothesisAnalyzer::analyze>:" << std::endl;
  ...
  if ( !genMEt->size() == 1 ) 
    throw cms::Exception("SVfitEventHypothesisAnalyzer::analyze") 
  ...
  for ( typename SVfitEventHypothesisCollection::const_iterator svFitEventHypothesis = svFitEventHypotheses->begin();
    ...
    if ( !((int)svFitEventHypothesis->numResonances() > idxResonance_) )
      throw cms::Exception("SVfitEventHypothesisAnalyzer") 
	<< " Failed to find resonance #" << idxResonance_ << " in SVfitEventHypothesis !!\n";
    const SVfitResonanceHypothesisBase* svFitResonanceHypothesis = svFitEventHypothesis->resonance(idxResonance_);
...
template <typename T>
void SVfitEventHypothesisAnalyzerT<T>::endJob()
{
  //std::cout << "<SVfitEventHypothesisAnalyzer::endJob>:" << std::endl;
...
typedef SVfitEventHypothesisAnalyzerT<SVfitEventHypothesis> SVfitEventHypothesisAnalyzer;
typedef SVfitEventHypothesisAnalyzerT<SVfitEventHypothesisByIntegration> SVfitEventHypothesisByIntegrationAnalyzer;

#include "FWCore/Framework/interface/MakerMacros.h"

DEFINE_FWK_MODULE(SVfitEventHypothesisAnalyzer);
DEFINE_FWK_MODULE(SVfitEventHypothesisByIntegrationAnalyzer);

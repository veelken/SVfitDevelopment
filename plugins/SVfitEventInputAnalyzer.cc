#include "TauAnalysis/SVfitDevelopment/plugins/SVfitEventInputAnalyzer.h"
...
SVfitEventInputAnalyzer::SVfitEventInputAnalyzer(const edm::ParameterSet& cfg)
...
SVfitEventInputAnalyzer::~SVfitEventInputAnalyzer()
{
  delete eventVertexFitAlgorithm_;
}

void SVfitEventInputAnalyzer::beginJob()
{
  if ( !edm::Service<DQMStore>().isAvailable() ) {
    throw cms::Exception("SVfitEventInputAnalyzer") 
...
void SVfitEventInputAnalyzer::analyze(const edm::Event& evt, const edm::EventSetup& es)
{
  ...
  if ( !(recMETs->size() == 1) )
    throw cms::Exception("SVfitEventInputAnalyzer::analyze") 
...
DEFINE_FWK_MODULE(SVfitEventInputAnalyzer);
